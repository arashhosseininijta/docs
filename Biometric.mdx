...
description: "Learn how to replace a pre-existing identity in audio file using Voice Harbor's public API using only cURL, while preserving Emotion and Phonetic information."
icon: user-shield
---
...
## 🎯 Biometric Identity Redaction via cURL (Voice Harbor V4)

Voice Harbor V4 exposes a secure HTTP API endpoint for fast biometric redaction of voice recordings. Below are minimal and advanced `curl` examples showing how to use the service.
...
### 🧬 Custom Biometric Identity Settings
...
- `gender`: same, opposite, random
- `language`: fr, es, de, english
- `model_id`: mini, advanced

---

### 🎭 Avatar-Based Redaction (English only)
...
Available avatars: mike, joe, emma, amelia
Available emotions: sad, angry, enunciated, happy, laughing, default

**Note:** When using `avatar` and `emotion`, you must not use `biometric-age` or `gender`.
...
### Invalid biometric-age

```json
HTTP 400 Bad Request
{ "error": "Invalid biometric-age. Allowed: young-adult, middle-aged-adult, senior-adult, same, random" }
```

### avatar/emotion used with non-English language

```json
HTTP 400 Bad Request
{ "error": "avatar/emotion only supported when language=english" }
```

### Mixing avatar/emotion with biometric-age/gender

```json
HTTP 400 Bad Request
{ "error": "avatar and emotion cannot be used together with biometric-age or gender" }
```
