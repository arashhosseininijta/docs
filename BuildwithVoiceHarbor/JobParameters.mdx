...
| 400 | Missing `jobId` path param (getJobContent) | `"Missing jobId"` |
...
| 429 | Rate limit exceeded (keyed by IP + token) | `"Too many requests, please slow down"` |
| 500 | DB query/insert/update failure | `"Database error"` |
| 500 | Cloud Run backend not configured (proxy attempt) | `"Cloud Run backend URL not configured"` |
| 500 | Proxy/upstream/network/timeout error while contacting Cloud Run | `"Anonymisation failed"` |
| 5xx | Upstream non-2xx status from Cloud Run | *Upstream status & body are forwarded as-is (streamed)* |
