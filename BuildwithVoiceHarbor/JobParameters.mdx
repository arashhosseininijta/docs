---
title: 'Job Parameters'
description: 'Customise your job based on your scenario'
icon: 'gear'
---

## Parameters

### File parameters


<ResponseField name="files" type="list" required>
List of local input audio file name(s).  

```yaml
files:
- filename_XYZ.mp3
- filename_XYZ.wav
```
</ResponseField>

<ResponseField name="prefix" type="string" default="">
Apply the task to the files with mentioned prefix in their file name.

```yaml
prefix: XYZ
```
</ResponseField>

### Task, model and agent parameters

</ResponseField>

<ResponseField name="task" type="string" default="protect">
  Task to apply to attached files.
  Available task's:
  - protect (PHI or/and Biomteric redaction*)
  - transcribe (Transcription and Biomteric redaction*)
  * If **biometric** parameter is activated. See **Biometric parameters** for more information.
```yaml
task: transcribe
```

</ResponseField>

<ResponseField name="model" type="string" default="mini">
Model tier to use for phi reduction. Pass `advanced` if you want to use advanced reasoning via our private-LLM. Available model's: advanced, mini.

<CardGroup cols={1}>
  <Card
    title="Advanced vs. Mini"
    icon="palette"
    href="https://docs.nijta.com/BuildwithVoiceHarbor/Model"
  >
    Choose the right model.
  </Card>
</CardGroup>

Example: 
```yaml
model: mini
```

Example: 
```yaml
model: advanced
```

<ResponseField name="agents" type="list" default="health-generic">
  Agents with specififc domain dependency and purpose.
```yaml
model: advanced
agents:
- hipaa
```
</ResponseField>

<CardGroup cols={1}>
  <Card
    title="Available Agents"
    icon="brain"
    href="https://docs.nijta.com/BuildwithVoiceHarbor/Model#advanced-model-labels"
  >
    Front-desk health calls or clinical reports, our agents take care of most sensitive Lables.
  </Card>
</CardGroup>

### Diarization and transcription parameters 

<ResponseField name="language" type="string" default="en">
  Set the target language for transcription.
  Available values for code-switching transcription:
  - de
  - en
  - fr
  - sp
```yaml
language: en
```

</ResponseField>

<ResponseField name="diar" type="boolean" default="false">
  If speaker diarization should be applied to your files. 
```yaml
diar: true
```

</ResponseField>

<ResponseField name="code-switch" type="boolean" default="false">
  If speaker diarization should be applied to your files. 
```yaml
code-switch: true
```

</ResponseField>

### Biometric parameters

<ResponseField name="biometric" type="string" default="en">
  If biometric voice print redaction should be applied to your files, and if so, which language.
  Available values:
  - en
  - fr
```yaml
biometric: en
```

</ResponseField>

<ResponseField name="biometric_gender" type="string" default="random">
  The gender which should be applied to the anonymised version.
  Available values:
  - random
  - same
  - opposite
```yaml
biometric_gender: random
```

</ResponseField>

<ResponseField name="biometric_age" type="string" default="middle-aged-adult">
  The age group which should be applied to the anonymised version.
  Available values:
  - young-adult (18-39)
  - middle-aged-adult (40-69)
  - senior-adult (70-100)
```yaml
biometric_age: middle-aged-adult
```

</ResponseField>


## Submit your Job using Voice Habor's SDK 

<Tip>

Speech to text is by **default** applied for the task **protect**. To have the transcription without any reduction use **transcribe** as task.  

</Tip>


<Steps>
  <Step title="Define your parameter based on data sensitivity.">
    [Choose the right model](https://docs.nijta.com/BuildwithVoiceHarbor/Model#advanced-vs-mini)
  </Step>
  <Step title="Build Job File">
    Build your job as yaml file containing the parameters in your target programming language.

    Minimal job example: 
    ```yaml
    files:
    - filename1.mp3
    - filename2.wav
    model: mini
    ```
  </Step>
  <Step title="Submit Job File">
    [Submit your job to trigger the task processing.](https://docs.nijta.com/api-reference/files/upload-url)
  </Step>
</Steps>


```python
BASE_URL = "https://voiceharbor.ai"
usage_token = "USAGE_TOKEN"
# Create a new job on the server via the class method.
job_id = VoiceHarborClient.create_job(BASE_URL, usage_token)

client = VoiceHarborClient(
    base_url=BASE_URL,
    job_id=job_id,
    token=usage_token,
    inputs_dir="./inputs/tests"
)

# Submit input files and the job file. 
job_params = {"files": [], "model":"mini"}  
job_params = client.submit_files(job_params)
job_file = client.submit_job(job_params)
logger.info(f"Job file created: {job_file}")
```

### Get Started with SDK

Start coding today using Python and integrate the Voice Harbor API into your workflows.

<CardGroup cols={1}>
  <Card
    title="Python SDK"
    icon="python"
    href="https://github.com/Nijta-API/VoiceHarborSDK"
  >
    Start using our Python SDK to connect with Voice Harbor and implement voice solutions
  </Card>
</CardGroup>



## Transcription, Translation, and Protection Use-Cases

## 1. Transcribe in 49 languages
Use this configuration to transcribe audio files in 49 languages.

```yaml
files:
  - filename_XYZ.mp3
  - filename_XYZ.wav
  - filename_XYZ.mp3
  - filename_ZYX.mp3
prefix: XYZ
task: translate
```

---

## 2. Transcribe in a Target Language
Use this configuration to transcribe audio files in a target language (e.g., English).

```yaml
files:
  - filename_XYZ.mp3
  - filename_XYZ.wav
  - filename_XYZ.mp3
  - filename_ZYX.mp3
prefix: XYZ
task: translate
language: en
```

---

## 3. Separate Speaker and Translate in Target Language
Use this configuration to separate speakers and translate to a target language.

```yaml
files:
  - filename_XYZ.mp3
  - filename_XYZ.wav
  - filename_XYZ.mp3
  - filename_ZYX.mp3
prefix: XYZ
task: translate
language: en
diar: true
```

---

## 4. Separate Speaker, Translate in 49 Languages
Use this configuration to separate speakers and translate in 49 languages.

```yaml
files:
  - filename_XYZ.mp3
  - filename_XYZ.wav
  - filename_XYZ.mp3
  - filename_ZYX.mp3
prefix: XYZ
task: translate
diar: true
```

---

## 5. Separate Speaker, Translate for Code-Switch Data in 4 Languages
Use this configuration to separate speakers and translate for code-switch data in 4 languages.

```yaml
files:
  - filename_XYZ.mp3
  - filename_XYZ.wav
  - filename_XYZ.mp3
  - filename_ZYX.mp3
prefix: XYZ
task: translate
diar: true
code-switch: true
```

---

## 6. Separate Speaker, Translate for Code-Switch Data in 4 Languages and Redact PHI with Mini Model
Use this configuration to separate speakers, translate for code-switch data in 4 languages, and redact PHI using the mini model.

```yaml
files:
  - filename_XYZ.mp3
  - filename_XYZ.wav
  - filename_XYZ.mp3
  - filename_ZYX.mp3
prefix: XYZ
task: protect
diar: true
code-switch: true
model: mini
```

---

## 7. Separate Speaker, Translate in 49 Languages and Redact PHI with Mini Model
Use this configuration to separate speakers, translate in 49 languages, and redact PHI using the mini model.

```yaml
files:
  - filename_XYZ.mp3
  - filename_XYZ.wav
  - filename_XYZ.mp3
  - filename_ZYX.mp3
prefix: XYZ
task: protect
diar: true
model: mini
```

---

## 8. Separate Speaker, Translate for Code-Switch Data in 4 Languages and Redact PHI with Advanced Model
Use this configuration to separate speakers, translate for code-switch data in 4 languages, and redact PHI using the advanced model. Includes HIPAA protection.

```yaml
files:
  - filename_XYZ.mp3
  - filename_XYZ.wav
  - filename_XYZ.mp3
  - filename_ZYX.mp3
prefix: XYZ
diar: true
code-switch: true
task: protect
model: advanced
agents:
  - hippa
  - health-generic
```

---

## 9. Separate Speaker, Translate in Target Language and Redact Biometric Voice Print
Use this configuration to separate speakers, translate in a target language, and redact biometric voice prints.

```yaml
files:
  - filename_XYZ.mp3
  - filename_XYZ.wav
  - filename_XYZ.mp3
  - filename_ZYX.mp3
prefix: XYZ
task: translate
language: en
diar: true
biometric: en
biometric_gender: random
biometric_age: middle-aged-adult
```

---

## 10. Separate Speaker, Translate in Target Language, Redact PHI, and Biometric Voice Print
Use this configuration to separate speakers, translate in a target language, redact PHI, and redact biometric voice prints using the advanced model.

```yaml
files:
  - filename_XYZ.mp3
  - filename_XYZ.wav
  - filename_XYZ.mp3
  - filename_ZYX.mp3
prefix: XYZ
biometric: en
biometric_gender: random
biometric_age: middle-aged-adult
task: protect
model: advanced
agents:
  - hippa
  - health-generic
diar: true
code-switch: true
```

---

### Explanation of Fields:

- **files**: List of audio files to be processed (can be in MP3, WAV, or other formats).
- **prefix**: Prefix for naming convention or logging.
- **task**: The main task to perform (e.g., `translate`, `protect`).
- **language**: Target language for transcription or translation (e.g., `en` for English).
- **diar**: Speaker separation (true/false).
- **code-switch**: Whether the audio contains code-switching between languages (true/false).
- **model**: The model to use for protecting sensitive data (e.g., `mini` or `advanced`).
- **agents**: Specific agents to apply for PHI protection (e.g., `hippa` for HIPAA protection).
- **biometric**: Language for biometric redaction (e.g., `en` for English).
- **biometric_gender**: Gender for biometric redaction (e.g., `random`, `female`, `male`).
- **biometric_age**: Age category for biometric redaction (e.g., `middle-aged-adult`).
