...
Only the following audio format is currently supported for input and output, in mono or stereo:
...
## Job Visibility for API Users

API jobs are created using a developer token and belong to the account that owns that token.

- **Developer-token-specific views** show jobs submitted with that token
- **Account-level job history** can include API jobs submitted under that account
